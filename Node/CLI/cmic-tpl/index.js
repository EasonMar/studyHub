...
module.exports = class extends Generator {
    ...
    writing() {
        const { framework, appName } = this.answers
        // copyTpl 会使用模板引擎，替换 <%= appName %> --- 把 模板插件包中的_package.json 替换关键字 并发布到 项目文件夹内
        this.fs.copyTpl(this.templatePath(framework, '_package.json'), this.destinationPath(appName, 'package.json'), this.answers)

        // copy 支持文件/文件夹 --- 把 对应模板下的文件 拷贝到 项目文件夹内
        fs.readdirSync(this.templatePath(framework))
            .filter(name => !name.startsWith('_'))
            .forEach(item => {
                this.fs.copy(this.templatePath(framework, item), this.destinationPath(appName, item))
            })
    }
    ...
    install() {
        ...
        this.log(chalk.green.bold('Happy coding!'))
    }
}
