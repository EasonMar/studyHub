...
type Log = <T>(p: T) => T;
let myLog: Log = log; // 为啥这里不需要传递【类型参数】: 这里泛型的意义仅在于约束输入输出类型一致, 而且类型别名没有提供传递泛型参数的入口...
myLog('string');
myLog(123);

type LogT<T> = (p: T) => T; // 这里就不一样了: 给类型别名加了泛型参数, 除了约束输入输出类型一致, 还得指定泛型具体是什么类型
...
// 这里仅仅是接口内部有个泛型函数(也没用提供传递泛型参数的入口)
interface iLog {
...
// 这个才是【真正的泛型接口】吧
// 使【泛型】可以约束接口的所有成员, 【泛型参数】在接口上(提供的传递泛型参数的入口)
interface iLogT<T> {
	(p: T): T;
}
